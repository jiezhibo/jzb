...
<body class="gray-bg">
			...
			<div class="col-lg-2">
				<div class="input-group">
					<span class="input-group-addon">学生姓名 </span> <input type="text"
						name="name" class="form-control" id="txt_search_username">
				</div>
			</div>
			<div class="col-lg-2">
				<div class="input-group">
					<span class="input-group-addon">代课老师</span> <select
						class="form-control" name="teacherid" id="txt_search_roleid">
					...
					<form id="form_user" method="post" action="reserveStudent.htm">
						...
						<table
							...
							<tr>
								...
								<td>&nbsp;&nbsp;</td>
								<td>所属年级：</td>
								<td colspan="4"><select class="form-control" name="grade"
									id="grade" aria-required="true" required>
							...
							<tr>
								...
								<td colspan="4"><select class="form-control"
									name="teacherid" id="teacherid" aria-required="true" required>
										<option value="">---请选择---</option>
										<c:forEach items="${tList }" var="t">
											<option value="${t.id }">${t.name }</option>
										</c:forEach>
								...
								<td>&nbsp;&nbsp;</td>
								<td>入学时间：</td>
								<td><input placeholder="入学时间" id="entrytime"
									name="entrytime" class="laydate-icon form-control layer-date" /></td>
							</tr>
						</table>
	...
	<!-- 新增和修改对话框 -->
	<div class="modal fade" id="modal_user_teacher" role="dialog"
		aria-labelledby="modal_user_teacher" aria-hidden="true">
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-body">
					<form id="form_teacher" method="post" action="reserveTeacher.htm">
						...
						<div class="modal-footer">
							<button type="button" class="btn btn-primary"
								id="submit_form_user_teacher">保存</button>
							<button type="button" class="btn btn-default"
								data-dismiss="modal">关闭</button>
						</div>
					</form>
	...
	<!--echarts对话框 -->
	<div class="modal fade" id="modal_user_echarts" role="dialog"
		aria-labelledby="modal_user_del" aria-hidden="true">
		<div class="modal-dialog">
			<div class="modal-content">
	...
	<script type="text/javascript">
		...
		$(function() {
			...
			$("#submit_form_user_teacher").click(function() {
				$("#form_teacher").submit();
			});
		});
		...
		var ButtonInit = function() {
			...
			oInit.Init = function() {
				...
				$("#btn_echarts")
						.click(
								function() {
									var techarts = echarts.init(document
											.getElementById("main"));
									var sz = new Array();
									var name = new Array();
									$('#modal_user_echarts').modal({
										backdrop : 'static',
										keyboard : false
									});
		...
		function initEditUser(getSelection) {
			...
			$('#entrytime').val(
					new Date(getSelection.entrytime)
							.Format("yyyy-MM-dd HH:mm:ss"));
